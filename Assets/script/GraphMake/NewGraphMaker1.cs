...
public class NewGraphMaker1 : MonoBehaviour
{
    ...
    void Update() //프레임마다 실행하는 함수
    {
        ...
        if (timePassed >= updateTime)
        {
            ...
            if (gameObject.name == "GraphMaker_A")
                EEG = EEGpow.GetComponent<FFTProcessor>().alphaPower;
            if (gameObject.name == "GraphMaker_B")
                EEG = EEGpow.GetComponent<FFTProcessor>().betaPower;
            if (gameObject.name == "GraphMaker_T")
                EEG = EEGpow.GetComponent<FFTProcessor>().thetaPower;
